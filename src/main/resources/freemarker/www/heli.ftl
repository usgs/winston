...
<script>
    ...
    if (heliW.value != "${DEFAULT_W}" && heliW.value != "") { 
		...
		if (${DEFAULT_SC?c} && heliSC.checked == "") {
			linkUrl += "&sc=0";
		} else if (!${DEFAULT_SC?c}  && heliSC.checked != "") {
      linkUrl += "&sc=1";
    }

		if (${DEFAULT_FC?c} && heliFC.checked == "") {
			linkUrl += "&fc=0";
		} else if (!${DEFAULT_FC?c} && heliFC.checked != "") {
      linkUrl += "&fc=1";
    }
	
		if (${DEFAULT_LB?c} && heliLB.checked == "") {
			linkUrl += "&lb=0";
		} else if (!${DEFAULT_LB?c} && heliLB.checked != "") {
      linkUrl += "&lb=1";
    }
...
<div class="tabContent">
	...
	<code>cv</code>: <b>Clip Value</b> Sets the number of counts above which to clip (default = ${DEFAULT_CV}).
		<br><br>
  <code>lb</code>: <b>Label</b> Whether to display a large label, 1 is yes, 0 is no (default = ${DEFAULT_LB?c}).
  	<br><br>
