<script>
    ...
    if (menuOB.value != "${DEFAULT_OB}") { 
    	linkUrl += "&ob=" + menuOB.value;
    }
    
    if (menuSO.value != "${DEFAULT_SO}") { 
    	linkUrl += "&so=" + menuSO.value;
    }

    if (menuTZ.value != "${DEFAULT_TZ}") { 
...
Returns the server menu. The menu contains the list of stations in this winston along with the least recent and most recent data point for each station.
...
<div class="tabContent">
	<code>ob</code>: <b>Order By</b> The column number used to order the menu (default = ${DEFAULT_OB}).<br>
	<br>
	<code>so</code>: <b>Sort Order</b> How to order the menu, a is ascending, d is decending (default = ${DEFAULT_SO}).<br>
	<br>
	<code>tz</code>: <b>Time Zone</b> (deafult = ${DEFAULT_TZ})<br>
	<br>
</div>
