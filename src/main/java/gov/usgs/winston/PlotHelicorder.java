...
public class PlotHelicorder {
	...
	private SimpleDateFormat dateFormat = new SimpleDateFormat(
			gov.usgs.util.Time.INPUT_TIME_FORMAT);
	...
	public PlotHelicorder(String[] a) {
		...
		WWSClient winston = new WWSClient(host, port);

		System.out.print("getting data...");
		String[] scn = settings.station.split("_");
		HelicorderData heliData = winston.getHelicorder(scn[0], scn[1], scn[2],
				scn[3], settings.startTime, settings.endTime, true);
		winston.close();
		...
		heliRenderer.setLocation(settings.left, settings.top, settings.xSize
				- (settings.left + settings.right), settings.ySize
	...
	public void outputInstructions() {
		...
		System.out
				.println("-e    end time [now], format: 'YYYYMMDDHHMMSS' (GMT) or 'now'");
