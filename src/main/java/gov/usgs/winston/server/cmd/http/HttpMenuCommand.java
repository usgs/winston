...
public final class HttpMenuCommand extends AbstractHttpCommand implements HttpBaseCommand {
    ...
    protected void sendResponse() {
        ...
        int sortCol = Util.stringToInt(arguments.get("ob"), DEFAULT_OB);
        if (sortCol < 1 || sortCol > 8) {
            writeSimpleHTML("Error: could not parse ob = " + arguments.get("ob"));
            return;
        }

        String o = Util.stringToString(arguments.get("so"), DEFAULT_SO);
    ...
    public String getUsage(HttpRequest req) {
        ...
        output.append("if (menuTZ.value != \"" + DEFAULT_TZ + "\") { linkUrl += \"&tz=\" + menuTZ.value;}\n");
        ...
        output.append("<select onchange=\"buildMenuUrl()\" class=\"timeZone\" id=\"menuTZ\" name=\"tz\" size=8>"
                + "</select>");
        ...
        output.append("<select id=\"menuOB\" onchange=\"buildMenuUrl()\" name=\"ob\">\n"
                ...
                + "<option value=7>Most Recent</option>" + "<option value=8>Type</Option>" + "</select></div><br>\n");
        ...
        output.append("<select id=\"menuSO\" onchange=\"buildMenuUrl()\" name=\"so\">"
                + "<option value=\"a\" selected>Ascending</option>" + "<option value=\"d\">Descending</option>"
                + "</select></div><br>");
        ...
        output.append("<div class=\"tabContent\">\n");
        output.append("<code>ob</code>: <b>Order By</b> The column number used to order the menu (default = "
                + DEFAULT_OB
                + ").<br><br>\n"
                + "<code>so</code>: <b>Sort Order</b> How to order the menu, a is ascending, d is decending (default = "
                + DEFAULT_SO + ").<br><br>\n" + "<code>tz</code>: <b>Time Zone</b> (deafult = " + DEFAULT_TZ
                + ")<br><br>\n");
        output.append("</div>");
        ...
        output.append("Most recently added channels:<br><a href=\"http://" + req.getHeader("Host")
                + "/menu?ob=6&so=d\">" + "http://" + req.getHeader("Host") + "/menu?ob=6&so=d</a><br><p><br>");
