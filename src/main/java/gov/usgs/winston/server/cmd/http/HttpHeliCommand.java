...
public final class HttpHeliCommand extends AbstractHttpCommand implements HttpBaseCommand {
    ...
    protected void sendResponse() {
        ...
        String tz = Util.stringToString(arguments.get("tz"), DEFAULT_TZ);
        ...
        if (settings.startTime == Double.NaN)
            error += "Error: cannot parse start time. Should be " + INPUT_DATE_FORMAT + " or -HH. I received "
                    + arguments.get("t1");
        ...
        int width = Util.stringToInt(arguments.get("w"), DEFAULT_W);
        int height = Util.stringToInt(arguments.get("h"), DEFAULT_H);
        ...
        if (error.length() > 0) {
            ...
            if (wws.getSlowCommandTime() > 0 && ct.getRunTimeMillis() > wws.getSlowCommandTime() * .75)
                wws.log(Level.INFO,
                        String.format(
                                ...
                                ct.getRunTimeMillis()), socketChannel);
            ...
            if (heliData == null || heliData.rows() <= 0)
                ...
                if (wws.getSlowCommandTime() > 0 && ct.getRunTimeMillis() > wws.getSlowCommandTime() * .75)
                    wws.log(Level.INFO,
                            String.format(
                                    "slow network (%1.2f ms) http/heli? " + settings.channel + " " + settings.startTime
                                            + " -> " + settings.endTime + " ("
                                            + decimalFormat.format(settings.endTime - settings.startTime) + ") ",
                                    ct.getRunTimeMillis()), socketChannel);
    ...
    public String getUsage(HttpRequest req) {
        ...
        output.append("<script>function buildHeliUrl() {" 
        ...
        output.append("if (heliT1.value != \"" + DEFAULT_T1 + "\" && heliT1.value != \"\") { linkUrl += \"&t1=\" + heliT1.value;}\n");
        output.append("if (heliT2.value != \"" + DEFAULT_T2 + "\" && heliT2.value != \"\") { linkUrl += \"&t2=\" + heliT2.value;}\n");
        output.append("if (heliW.value != \"" + DEFAULT_W + "\" && heliW.value != \"\") { linkUrl += \"&w=\" + heliW.value;}\n");
        output.append("if (heliH.value != \"" + DEFAULT_H + "\" && heliH.value != \"\") { linkUrl += \"&h=\" + heliH.value;}\n");
        output.append("if (heliTC.value != \"" + DEFAULT_TC + "\" && heliTC.value != \"\") { linkUrl += \"&tc=\" + heliTC.value;}\n");
        output.append("if (heliBR.value != \"" + DEFAULT_BR + "\" && heliBR.value != \"\") { linkUrl += \"&br=\" + heliBR.value;}\n");
        output.append("if (heliCV.value != \"" + DEFAULT_CV + "\" && heliCV.value != \"\") { linkUrl += \"&cv=\" + heliCV.value;}\n");
        ...
        output.append("if (heliTZ.value != \"" + DEFAULT_TZ + "\") { linkUrl += \"&tz=\" + heliTZ.value;}\n");
        ...
        output.append("<P><br><p><div class=\"left\"><div class=\"timeInput\"><label for=\"t1\">Start Time</label></div><input type=text id=\"heliT1\" onchange=\"buildHeliUrl()\" name=\"t1\" size=10 value=\"" + DEFAULT_T1 + "\"></div><br>");
        output.append("<div class=\"clear\"></div>\n");
        output.append("<div class=\"left\"><div class=\"timeInput\"><label for=\"t2\">End Time</label></div><input type=text id=\"heliT2\" onchange=\"buildHeliUrl()\" name=\"t2\" size=10 value=\"" + DEFAULT_T2 + "\"></div><br>");
        output.append("</div>\n");
        output.append("<div class=\"right\" style=\"padding-right:15px;\">\n");
        output.append("<div class=\"input\"><label for=\"w\">Width</label><input type=text id=\"heliW\" onchange=\"buildHeliUrl()\" name=\"w\" size=3 value=\"" + DEFAULT_W + "\"></div>");
        output.append("<div class=\"input\"><label for=\"h\">Height</label><input type=text id=\"heliH\" onchange=\"buildHeliUrl()\" name=\"h\" size=3 value=\"" + DEFAULT_H + "\"></div>");
        output.append("<div class=\"input\"><label for=\"tc\">Time Chunk</label><input type=text id=\"heliTC\" onchange=\"buildHeliUrl()\" name=\"tc\" size=3 value=\"" + DEFAULT_TC + "\"></div>");
        output.append("<div class=\"input\"><label for=\"br\">Bar Range</label><input type=text id=\"heliBR\" onchange=\"buildHeliUrl()\" name=\"br\" size=3 value=\"" + DEFAULT_BR + "\"></div>");
        ...
        output.append("<div class=\"input\"><label for=\"cv\">Clip Value</label><input type=text id=\"heliCV\" onchange=\"buildHeliUrl()\" name=\"cv\" size=3 value=\"" + DEFAULT_CV + "\"></div>");
