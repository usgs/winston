...
public final class HttpGapsCommand extends AbstractHttpCommand implements HttpBaseCommand {
    ...
    protected void sendResponse() {
        ...
        if (startTime == Double.NaN) {
            error = error + "Error: could not parse start time (t1). Should be " + INPUT_DATE_FORMAT + " or -HH.";
        }
    ...
    void writeHuman() {
        ...
        outputSettings_Analysis.append("<TBODY><TR><TD>Start Time:</TD><TD>" + startTimeS + "</TD></TR>");
        ...
        outputSettings_Analysis.append("<TR><TD>Total Gap Length:</TD><TD>" + formatter.format(totalGapLength)
                + "seconds</TD></TR>");
        outputSettings_Analysis.append("<TR><TD>Average Gap Length:</TD><TD>" + formatter.format(averageGapLength)
                + "seconds</TD></TR>");
        outputSettings_Analysis.append("<TR><TD>Data percentage:</TD><TD>" + formatter.format(dataPercentage)
                + "%</TD></TR></TABLE></TD>");
        ...
        // Website Creation
        String html = header.toString() + outputSettings_Analysis.toString() + outputDataGaps.toString()
                + footer.toString();
    ...
    public String getUsage(HttpRequest req) {
        ...
        output.append("if (gapsTZ.value != \"" + DEFAULT_TZ + "\") { linkUrl += \"&tz=\" + gapsTZ.value;}\n");
        ...
        output.append("<select onchange=\"buildGapsUrl()\" class=\"timeZone\" id=\"gapsTZ\" name=\"tz\" size=8>"
                + "</select></div>\n");
        output.append("<div class=\"clear\"></div>");
        output.append("<P><br><p><div class=\"label\"><label for=\"t1\">Start Time</label></div><input type=text id=\"gapsT1\" onchange=\"buildGapsUrl()\" name=\"t1\" size=10 value=\""
                + DEFAULT_T1 + "\"><br>");
        output.append("<div class=\"label\"><label for=\"t2\">End Time</label></div><input type=text id=\"gapsT2\" onchange=\"buildGapsUrl()\" name=\"t2\" size=10 value=\""
                + DEFAULT_T2 + "\"><br>");
        output.append("</div>\n");
        output.append("<div class=\"right\" style=\"padding-right:15px;\">\n");
        output.append("<div class=\"input\"><label for=\"mgd\">Min Gap</label><input type=text id=\"gapsMGD\" onchange=\"buildGapsUrl()\" name=\"mgd\" size=3 value=\""
                + DEFAULT_MGD + "\"></div>");
