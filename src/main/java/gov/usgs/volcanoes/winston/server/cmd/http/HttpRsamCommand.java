...
public final class HttpRsamCommand extends AbstractHttpCommand implements HttpBaseCommand {
    ...
    private String parseArguments() {
        ...
        if (endTime == Double.NaN)
            errorString += "Error: could not parse end time (t2). Should be " + INPUT_DATE_FORMAT + ".<br>";

        startTime = getStartTime(arguments.get("t1"), endTime, ONE_DAY);
        if (startTime == Double.NaN)
            errorString += "Error: could not parse start time (t1). Should be " + INPUT_DATE_FORMAT + " or -HH.M<br>";
    ...
    private void getData() {
        ...
        try {
            ...
            rsamData = data.getRSAMData(code, startTime, endTime, 0, dst, dsInt);
    ...
    private void sendPlot() {
        ...
        MatrixRenderer mr = new MatrixRenderer(rsamData.getData(), false);
        double max = Math.min(plotMax, rsamData.max(1) + rsamData.max(1) * .1);
        double min = Math.max(plotMin, rsamData.min(1) - rsamData.max(1) * .1);
    ...
    public String getUsage(HttpRequest req) {
       ...
       output.append("if (rsamW.value != \"" + DEFAULT_W
        ...
        if (DEFAULT_DS) {
            output.append("if (rsamDS.checked == \"\") { linkUrl += \"&ds=0\"; } else if (rsamDSP.value != \""
                    + DEFAULT_DSP + "\" && rsamDSP.value != \"\") { linkUrl += \"&dsp=\" + rsamDSP.value;}\n");
        } else {
            output.append("if (rsamDS.checked != \"\") { linkUrl += \"&ds=1\"; if (rsamDSP.value != \"" + DEFAULT_DSP
                    + "\" && rsamDSP.value != \"\") { linkUrl += \"&dsp=\" + rsamDSP.value;}}\n");
        }

        if (DEFAULT_RM) {
            output.append("if (rsamRM.checked == \"\") { linkUrl += \"&rm=0\";} else if (rsamRMP.value != \""
                    + DEFAULT_RMP + "\" && rsamRMP.value != \"\") { linkUrl += \"&rmp=\" + rsamRMP.value;}\n");
        } else {
            output.append("if (rsamRM.checked != \"\") { linkUrl += \"&rm=1\"; if (rsamRMP.value != \"" + DEFAULT_RMP
                    + "\" && rsamRMP.value != \"\") { linkUrl += \"&rmp=\" + rsamRMP.value;}}\n");
        }

        output.append("if (rsamTZ.value != \"" + DEFAULT_TZ + "\") { linkUrl += \"&tz=\" + rsamTZ.value;}\n");
        ...
        output.append("<select onchange=\"buildRsamUrl()\" class=\"timeZone\" id=\"rsamTZ\" name=\"tz\" size=8>"
                + "</select></div>\n");
        output.append("<div class=\"clear\"></div>");
        output.append("<P><br><p><div class=\"left\"><div class=\"timeInput\"><label for=\"t1\">Start Time</label></div><input type=text id=\"rsamT1\" onchange=\"buildRsamUrl()\" name=\"t1\" size=10 value=\""
                + DEFAULT_T1 + "\"></div><br>");
        output.append("<div class=\"left\"><div class=\"timeInput\"><label for=\"t2\">End Time</label></div><input type=text id=\"rsamT2\" onchange=\"buildRsamUrl()\" name=\"t2\" size=10 value=\""
                + DEFAULT_T2 + "\"></div><br>");
        output.append("</div>\n");
        output.append("<div class=\"right\" style=\"padding-right:15px;\">\n");
        output.append("<div class=\"input\"><label for=\"rsamP\">RSAM period</label><input type=text id=\"rsamP\" onchange=\"buildRsamUrl()\" name=\"rsamP\" size=3 value=\""
                + DEFAULT_RSAMP + "\"></div>");
        output.append("<div class=\"input\"><label for=\"w\">Width</label><input type=text id=\"rsamW\" onchange=\"buildRsamUrl()\" name=\"w\" size=3 value=\""
                + DEFAULT_W + "\"></div>");
        output.append("<div class=\"input\"><label for=\"h\">Height</label><input type=text id=\"rsamH\" onchange=\"buildRsamUrl()\" name=\"h\" size=3 value=\""
                + DEFAULT_H + "\"></div>");
        output.append("<div class=\"input\"><label for=\"max\">Plot Max</label><input type=text id=\"rsamMAX\" onchange=\"buildRsamUrl()\" name=\"max\" size=3 value=\""
                + DEFAULT_MAX + "\"></div>");
        output.append("<div class=\"input\"><label for=\"min\">Plot Min</label><input type=text id=\"rsamMIN\" onchange=\"buildRsamUrl()\" name=\"min\" size=3 value=\""
                + DEFAULT_MIN + "\"></div>");
        ...
        output.append("<div class=\"input\"><label for=\"dsp\">Period</label><input type=text id=\"rsamDSP\" onchange=\"buildRsamUrl()\" name=\"pds\" size=3 value=\""
                + DEFAULT_DSP + "\"></div>");
       ...
       output.append("</div><br></FORM><div class=\"clear\"></div>\n");
        ...
        output.append("<div class=\"tabContent\">\n");
        output.append("The options (separated by the & character, all optional except for <code>code</code> ) are defined as follows:<br><br>"
