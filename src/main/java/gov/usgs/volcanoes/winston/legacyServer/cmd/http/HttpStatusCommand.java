...
public final class HttpStatusCommand extends AbstractHttpCommand implements HttpBaseCommand {
  ...
  protected void sendResponse() {
    ...
    for (int i = 0; i < sts.size(); i++) {
      ...
      if (wws.getMaxDays() > 0 && J2kSec.fromEpoch(timeSpan.endTime) < now - (wws.getMaxDays() * (60 * 60 * 24))) {
        sts.remove(i);
        i--;
      }
    }
    ...
    Channel chan1 = sts.get(sts.size() / 2);
    TimeSpan timeSpan = chan1.timeSpan;
    final double medianDataAge = now - J2kSec.fromEpoch(timeSpan.endTime);
    ...
    for (final Channel chan : sts) {
      TimeSpan timeSpan1 = chan.timeSpan;
      final double age = now - J2kSec.fromEpoch(timeSpan1.endTime);
    ...
    double et = J2kSec.fromEpoch(chan.timeSpan.endTime);
    output.append("<TR><TD>most recent</TD><TD>" + chan.scnl.toString("_") + " "
        + formatter.format(now - et) + " seconds ago</TD></TR>");
    output.append(
    ...
    int i = 0;
    for (final Channel chan2 : sts) {
      final double a = now - J2kSec.fromEpoch(chan2.timeSpan.endTime);
      if (a > (5 * 60) && a < (60 * 60 * 24)) {
        final String bg = i++ % 2 == 0 ? "#eeeeff;" : "#ffffff;";
        output.append("<TR STYLE=\"background: " + bg + "\"><TD ALIGN=right>"
            + chan2.scnl.toString("_") + "</TD><TD>" + ((int) a / 60)
            + " minutes</TD></TR>");
      }
    }
