...
public class WaveServerEmulator {
  ...
  public int getChannelID(final String s, final String c, final String n, final String l) {
    ...
    final int id = channels.getChannelID(new Scnl(s, c, n, loc));
    return id;
  }
  ...
  public List<String> getWaveServerMenu(final boolean isScnl, final double embargo,
      final double span,
    ...
    for (final Channel st : sts) {
      ...
      if (!isScnl) {
        list.add(" " + st.sid + " " + scnl.station + " " + scnl.channel + " " + scnl.network + " "
            + decimalFormat.format(Ew.fromEpoch(timeSpan.startTime)) + " "
            + decimalFormat.format(Ew.fromEpoch(timeSpan.endTime)) + " s4 ");
      } else {
        final String line = " " + st.sid + " " + scnl.toString(" ")
            + " " + decimalFormat.format(Ew.fromEpoch(timeSpan.startTime)) + " "
            + decimalFormat.format(Ew.fromEpoch(timeSpan.endTime)) + " s4 ";
        list.add(line);
      }
...
//    final String trueCode = s + "$" + c + "$" + n + loc;
  ...
  public Object[] getWaveServerRaw(final String s, final String c, final String n, final String l,
    ...
    try {
      final int sid = channels.getChannelID(new Scnl(s, c, n, lc));
