...
public class Upgrade {
  ...
  private final Upgrader[] upgraders = new Upgrader[] {new Upgrader1_1_0to1_1_1(),
      new Upgrader1_0_1to1_1_0(), new Upgrader1_0_0to1_0_1()};
  ...
  private class Upgrader1_1_0to1_1_1 extends Upgrader {
    ...
    public void upgrade() {
      try {
        ...
        winston.getStatement().execute("ALTER TABLE channelmetadata " + "DROP PRIMARY KEY, "
            + "ADD PRIMARY KEY (sid, name), " + "DROP cmid");
    ...
    public void upgrade() {
      try {
        ...
        winston.getStatement()
            .execute("CREATE TABLE instruments (" + "iid INT PRIMARY KEY AUTO_INCREMENT,"
                + "name VARCHAR(128) UNIQUE, " + "description VARCHAR(255), "
                + "lon DOUBLE DEFAULT -999, " + "lat DOUBLE DEFAULT -999, "
                + "height DOUBLE DEFAULT -999, " + "timezone VARCHAR(128))");

        System.out.println("Altering channels table...");
        winston.getStatement()
            .execute("ALTER TABLE channels " + "ADD COLUMN iid INT AFTER sid, "
        ...
        winston.getStatement()
            .execute("CREATE TABLE grouplinks (glid INT PRIMARY KEY AUTO_INCREMENT, "
                + "sid INT, nid INT) " + winston.tableEngine);
        winston.getStatement()
            .execute("CREATE TABLE groupnodes (nid INT PRIMARY KEY AUTO_INCREMENT, "
                + "parent INT DEFAULT 0, " + "name CHAR(255), " + "open BOOL DEFAULT 0) "
                + winston.tableEngine);

        System.out.println("Creating channel metadata table...");
        winston.getStatement()
            .execute("CREATE TABLE channelmetadata (" + "cmid INT PRIMARY KEY AUTO_INCREMENT, "
                + "sid INT, " + "name VARCHAR(255), " + "value TEXT) " + winston.tableEngine);
        ...
        winston.getStatement()
            .execute("CREATE TABLE instrumentmetadata (" + "imid INT PRIMARY KEY AUTO_INCREMENT, "
                + "iid INT, " + "name VARCHAR(255), " + "value TEXT) " + winston.tableEngine);
    ...
    public void upgrade() {
      try {
        ...
        winston.getStatement().execute(
            "ALTER TABLE channels ADD COLUMN lon DOUBLE DEFAULT -999, ADD COLUMN lat DOUBLE DEFAULT -999");
  ...
  public Upgrader getUpgrader() {
    ...
    if (winston.checkConnect()) {
      final String currentSchemaVersion = winston.getSchemaVersion();
  ...
  public void doUpgrade() {
    final Upgrader ugr = getUpgrader();
    if (ugr == null) {
  ...
  public static void main(final String[] args) {
    final Upgrade ug = new Upgrade();
    if (args.length == 1 && args[0].equals("--upgrade")) {
      ug.doUpgrade();
    } else {
      final String currentSchemaVersion = ug.getCurrentVersion();
      System.out.println("Current Winston schema version: " + currentSchemaVersion);
      System.out.println();
      final String upDescription = ug.getUpgraderDescription();
